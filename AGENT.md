...
- **`--clear-cache`** (live): clears **`ffprobe-tile-v1`**, **`ffprobe-tile-v2`**, **`ffprobe-tile-v3`**, **`ffprobe-tile-v4`**, and **`tile-randomized`** under **`~/.cache/mpv-img-tricks/`** — from Python for basic live and plain **`--render`**, forwarded as **`--clear-cache`** to **`img-effects.sh`** for tile, chaos, and render-with-effect.
