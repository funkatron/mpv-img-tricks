...
2. **`filter_tile_readable_inputs`** (**validate-media**): optional **`ffprobe`** on each playlist entry; uses **`run_under_nice`** and **`-threads 1`** like composite **`ffmpeg`**. Probe accepts **any video stream**, then extension-scoped demux checks, then **any** demuxable file (odd/recovered extensions). Results are cached under **`~/.cache/mpv-img-tricks/ffprobe-tile-v3/`** with **MD5** when **`md5`/`md5sum`** exist; cache keys use **`stat`** (device, inode, size, mtime) and **not** paths — when **inode is 0** or **`stat` fails**, the key also includes an **MD5 of the first 64KiB** so exFAT-style layouts still disambiguate. Set **`MPV_IMG_TRICKS_NO_FFPROBE_TILE_CACHE`** to bypass. Parallelism uses the same **`JOBS`** / **`resolve_parallel_job_count_for_tile`** rule as compositing.
3. **Sound**
