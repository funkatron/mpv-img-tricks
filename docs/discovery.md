...
2. **`filter_tile_readable_inputs`** (**validate-media**): optional **`ffprobe`** on each playlist entry; uses **`run_under_nice`** and **`-threads 1`** like composite **`ffmpeg`**. Probe accepts **any video stream** (not only **`v:0`**), with a demux fallback for known image extensions. Results are cached under **`~/.cache/mpv-img-tricks/ffprobe-tile-v2/`** with **MD5** cache filenames when **`md5`/`md5sum`** exist (**SHA-256** fallback); the key includes the **file path** plus **`stat`** (**dev**, **inode**, **size**, **mtime**) so volumes with useless inodes (e.g. exFAT) do not collapse every file into one entry. Parallelism uses the same **`JOBS`** / **`resolve_parallel_job_count_for_tile`** rule as compositing (**half CPU cores** by default, capped).
3. **Sound**
