...
- **`--tile-motion-strength`** (default **`1.0`**, must be **positive**): scales motion intensity; try **`0.5`**–**`2.0`** for tuning. The implementation also applies a fixed **0.5×** global drift scale on pan and zoom ramp so motion stays slower than the raw coefficients. Motion is rendered at **60 fps** inside each slide clip; longer **`--duration`** (e.g. **4–8** seconds) makes pan/zoom easier to see on large grids.
